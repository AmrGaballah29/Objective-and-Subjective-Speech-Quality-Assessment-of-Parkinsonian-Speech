...
%Then we complete traing the objective models using the new feature sets.
...
%This script is to train MFCC objective models. The used machine algorithms
%are linear regression (LR), support vector regression (SVR), Gaussian
%process regression (GPR), and deep neural networks (DNN). These models are
%utilizing the whole MFCC feature set.
clear;close all;clc;
load('MFCC_Full');
...
%This script is to train reduced MFCC objective models. Instead of using
%the whole MFCC features and being proned to overfitting, a feature
%reduction agorithm is applied on the training matrix before training the objective models. 
%please check the article " Objective and Subjective Speech Quality
%Assessment of Amplification Devices for Patients with Parkinson's
%Disease".
%This is to reduce the number of MFCC features from 26 features to 16
%features
[MSE_train,MSE_test,M] = Select_Features(MFCC_Training_mx,MFCC_Training_labels);
MFCC_Red_Training_mx=MFCC_Training_mx(:,M(1:16));
MFCC_Red_Test_mx=MFCC_Test_mx(:,M(1:16));
...
%This script is to train GFCC objective models. The used machine algorithms
%are linear regression (LR), support vector regression (SVR), Gaussian
%process regression (GPR), and deep neural networks (DNN). These models are
%utilizing the whole GFCC feature set.
clear;close all;clc;
load('GFCC_Full');
...
%This script is to train reduced GFCC objective models. Instead of using
%the whole GFCC features and being proned to overfitting, a feature
%reduction agorithm is applied on the training matrix before training the objective models. 
%please check the article " Objective and Subjective Speech Quality
%Assessment of Amplification Devices for Patients with Parkinson's
%Disease".
%This is to reduce the number of GFCC features from 60 features to 11
%features
[MSE_train,MSE_test,M] = Select_Features(GFCC_Training_mx,GFCC_Training_labels);
GFCC_Red_Training_mx=GFCC_Training_mx(:,M(1:11));
GFCC_Red_Test_mx=GFCC_Test_mx(:,M(1:11));
